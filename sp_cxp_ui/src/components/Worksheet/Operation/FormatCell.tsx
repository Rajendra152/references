...
const widthOptions: { [widthName: string]: string } = {
  ["10"]: '"10"',
  ["12"]: '"12"',
  ["14"]: '"14"',
  ["18"]: '"18"',
...
const colWidth = Object.keys(widthOptions);
const optionsColWidth: IComboBoxOption[] = colWidth.map((widthName: string) => ({
  key: widthName,
  text: widthName,
  styles: {
    optionText: {
      fontFamily: widthOptions[widthName],
      overflow: 'visible',
      whiteSpace: 'normal',
    },
  },
...
const rowHeightOptions: { [heightName: string]: string } = {
  ["8"]: '"8"',
  ["10"]: '"10"',
  ["12"]: '"12"',
  ["14"]: '"14"',
...
const rowHeight = Object.keys(rowHeightOptions);
const optionsRowWidth: IComboBoxOption[] = rowHeight.map((heightName: string) => ({
  key: heightName,
  text: heightName,
  styles: {
    optionText: {
      fontFamily: widthOptions[heightName],
      overflow: 'visible',
      whiteSpace: 'normal',
    },
  },
...
const FormatCell: React.FunctionComponent = (props) => {
  ...
  useEffect(() => {
    // setHeight(
    //   props.optionsState.optionsCollection.Worksheet.appearance.rowHeight
    // );
    // setWidth(
    //   props.optionsState.optionsCollection.Worksheet.appearance.colWidth
    // );
   
    
    setDecimal(
      props.optionsState.optionsCollection.Worksheet.numeric.decimalPlaces
    );
    if (
      props.optionsState.optionsCollection.Worksheet.numeric.displayAs.includes(
        "E Notation When Needed"
      )
    ) {
      setDisplay("1");
    } else if (
  ...
  // };
  const [selectedColKey, setSelectedColKey] = React.useState<string | undefined>('');
  const [selectedRowKey, setSelectedRowKey] = React.useState<string | undefined>('');
  const [errorMessage, setErrorMessage] = React.useState<string | undefined>('');
  const [rowErrorMessage, setRowErrorMessage] = React.useState<string | undefined>('');

  const _onChangeCol: IComboBoxProps['onChange'] = (event, option) => {
    console.log(event, option)
    if (option?.key) {
      setSelectedColKey(option!.key as string);
      setWidth(option!.key as string);
      setOptions("Worksheet.appearance.colWidth", option!.key as string, props);
      setErrorMessage('');
    }
  }
  ...
  //   console.log(item);
  //   }
  const _onChangeRowHeight: IComboBoxProps['onChange'] = (event, option) => {
    console.log(event, option)
    if (option?.key) {
      setSelectedRowKey(option!.key as string);
      setHeight(option!.key as string);
      setOptions("Worksheet.appearance.rowHeight", option!.key as string, props);
      setRowErrorMessage('');
    }
  }
  ...
  const cellFormatter = (data) => {
    ...
    if (data.lastHeader === "Rows and Columns") {
      ...
      console.log('Height format cells', height)
      ...
      // props.referenceObjectState.setRowHeight(data.height, i, 0);
      {
        ...
        props.referenceObjectState.setRowHeight(height * 2, i);
      }
      ...
      //
      setTimeout(() => {
        setSelectedRowKey("12");
        setSelectedColKey("24");
      },500)
      ...
      if (data.type === "Numeric" && data.decimal) {
        ...
        if (data.display === "4") {
          let val;
          props.referenceObjectState.numberFormat("General", range);
          // props.referenceObjectState.getData(range).then((data) => {
      ...
      if (data.type === "Text") {
        ...
        props.referenceObjectState.wrap(range, isChecked);
        props.actions.formatCellAction.isOpenFormatCell({
          message: false,
        });
        props.referenceObjectState.actionComplete();
      ...
      if (data.type === "Accounting") {
        ...
        props.referenceObjectState.numberFormat(
      ...
      >
        ...
        <div className={contentStyles.header} style={{ display: "flex", justifyContent: 'space-between' }}>
          ...
          <div style={{ display: "flex", flexDirection: "row" }}>
            <div className="ms-Grid-col ms-sm3 " style={{ marginLeft: "60px" }}>
              <Helpbutton nodeId="format_cells__data" />
            </div>
            <IconButton
                    ...
                    <Stack>
                      <div className='ms-Grid-col ms-sm6 ms-md4 ms-lg12'>
                        ...
                        <ComboBox
                          ...
                          selectedKey={selectedColKey}
                          options={optionsColWidth}
                          allowFreeform
                          autoComplete='on'
                          ...
                          onPendingValueChanged={(option, pendingIndex, pendingValue) => {
                            if (option || pendingValue) {
                              console.log(option, pendingValue)
                              ...
                              console.log("inside else------>", isNaN(pendingValue))
                              console.log("Col width :", colWidth.includes(String(pendingValue)), colWidth)
                              if (widthOptions[pendingValue] || pendingValue == "12" || pendingValue == "14" || pendingValue == "18") {
                                setErrorMessage('')
                                console.log("width options:" + widthOptions[pendingValue])
                                // console.log("Col width :", colWidth.includes(pendingValue))
                                // colWidth, Object.entries(colWidth), typeof (colWidth)
                                setSelectedColKey(pendingValue as string);
                                setWidth(pendingValue as string);
                                setOptions("Worksheet.appearance.colWidth", pendingValue as string, props);
                              }
                              else if (option?.key) {
                                setErrorMessage('')
                              }
                              else {
                                setErrorMessage('Enter valid option')
                              }
                            }
                          }
                          }
                          onChange={_onChangeCol}
                        />
                      </div>
                    </Stack>
                    {/* style={{ lineHeight: 4 }} */}
                    ...
                    <Stack>
                      <div className='ms-Grid-col ms-sm6 ms-md4 ms-lg12'>
                        ...
                        <ComboBox
                          ...
                          options={optionsRowWidth}
                          allowFreeform
                          autoComplete='on'
                          //  key={'' + autoComplete + allowFreeform}
                          //  styles={comboBoxStyles}
                          errorMessage={rowErrorMessage == '' ? undefined : rowErrorMessage}
                          onPendingValueChanged={(option, pendingIndex, pendingValue) => {
                            if (option || pendingValue) {
                              console.log(option, pendingValue)
                              console.log(`Preview value was changed. Pending index: ${pendingIndex} Pending value: ${pendingValue}`)
                              ...
                              if (widthOptions[pendingValue] || pendingValue == "12" || pendingValue == "14" || pendingValue == "18") {
                                ...
                                setRowErrorMessage('Enter valid option')
                              }
                            }
                          }
                          }
                          onChange={_onChangeRowHeight}
                        />
                      </div>
                    </Stack>
                    <Stack className='label-text' >
          ...
          <Stack horizontal tokens={stackTokens} styles={stackStyles}>
            <Stack {...columnProps}>
              <DefaultButton
                ...
                allowDisabledFocus
                disabled={disabled || errorMessage || rowErrorMessage}
                checked={checked}
                onClick={() => {
                  updateGrid()
                  setErrorMessage('')
