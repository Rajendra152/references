...
const BulkDeleteRow: React.FunctionComponent = (props) => {
  ...
  const _onChangeCol = (option: IDropdownOption, index?: number) => {
    ...
    bulkInsert.atRow = parseInt(option.key);
  ...
  const _onChangeFindInput = (e: React.FormEvent<HTMLElement>) =>{
    ...
    bulkInsert.deleteRow = e.target.value;
    setBulkInsert(bulkInsert);
  }
  ...
  const _handleDeleteRow = () => {
    ...
    console.log(typeof( bulkInsert.atRow) , typeof(bulkInsert.deleteRow),  parseInt(bulkInsert.atRow - 1)+parseInt(bulkInsert.deleteRow),  " bulkInsert.before+bulkInsert.insertRow",  parseInt(bulkInsert.atRow - 1)+parseInt(bulkInsert.deleteRow))
      props.referenceObjectSpreadSheet.instance.delete( bulkInsert.atRow,  parseInt(bulkInsert.atRow - 1)+parseInt(bulkInsert.deleteRow), "Row")
      props.actions.deleteAction.isOpenDelRow({message: false})
  }

  return (
    <div>

      {/* <DefaultButton secondaryText="Find" className={`text-block`} onClick={showModal} text="Delete Row" /> */}
      <Modal
        titleAriaId={'title1'}
        // isOpen={isModalOpen}
        // onDismiss={hideModal}
        isOpen={props.isOpen}
        onDismiss={()=>props.actions.deleteAction.isOpenDelRow({message: false})}
        isModeless={true}
        containerClassName={contentStyles.container}
        dragOptions={dragOptions}
      >
        <div className={contentStyles.header}>
          <span id={'title1'}>Delete Row</span>
          <IconButton
            styles={iconButtonStyles}
            iconProps={cancelIcon}
            ariaLabel="Close popup modal"
            // onClick={hideModal}
            onClick={()=>props.actions.deleteAction.isOpenDelRow({message: false})}
          />
        </div>
...
export default connect(mapStateToProps, mapDispatchToProps)(BulkDeleteRow)
