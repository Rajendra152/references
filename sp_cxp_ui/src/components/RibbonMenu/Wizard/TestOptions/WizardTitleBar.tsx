...
import * as storeSpreadsheetAction from "../../../../store/Worksheet/SpreadSheet";
import { bindActionCreators } from "redux";
import { closeWindow, sendtoAnalysisMenu } from "../DataTransfer/DataTransfer";
import { remote} from 'electron'
import * as analysisOptions from "../../../../store/Analysis/actions";
...
> {
  ...
  componentDidUpdate(prevProps: IWizardTitleBarProps) {
    if (
      ...
      prevProps.stepType !== this.props.stepType
    ) {
      this.makeTitle();
  ...
  makeTitle = () => {
    ...
    if (this.props.stepType == "DATASELECTION") {
      ...
      if (this.props.title === "Principal Components") {
        ...
        this.props.title === "LogRank" ||
        this.props.title === "Gehan-Breslow"
      ) {
        option = " - Select Groups";
      } else if (
        this.props.title === "Proportional Hazards" ||
        this.props.title === "Stratified Model"
      ) {
  ...
  handleCloseClick = () => {
   // console.log('handle close done');
    this.props.analysisOptions.isTestOptionDisabled({message:false})
    let data ={
      testOptionDisabled: this.props.isTestOptionDisabled
    }
...
function mapDispatchToProps(dispatch) {
  return {
    analysisOptions: bindActionCreators(analysisOptions, dispatch),
