...
import { getAllColor, getFillDirection, getFillColorScale } from "../../../../utils/graphDailogProperty/differentPropertyList";
...
const Areafill = (props) => {
  ...
  const [fillColorScheme, setFillColorScheme] = useState(props.areaProp.fillColorScheme);
  const [Identify, setIdentify] = useState(true);
  const [gradient, setgradient] = useState(true);
  const [endColor, setEndColor] = useState("white ");
  const [endColorScheme, setEndColorScheme] = useState("white ");
  ...
  const options = useRef({
    ...
    directionOptions:getFillDirection()
})

  useEffect(()=>{
    console.log("hhhh")
    setColor(props.areaProp.fillcolor)
    setFillColorScheme(props.areaProp.fillColorScheme)
    setDirection(props.areaProp.fill === "none" ? "tozeroy" : props.areaProp.fill)
    setEndColor("#ffffff")
  ...
  useEffect(() => {
    props.OpenHelpWindow("wbasics", "pub_dtlgraph_properties__graph", "graph_properties__area_fills")
  ...
  const colorOnChange = (value) => {
    const newProps = {
      areaFills: {
        ...props.areaProp,
        fillcolor: value,
        fill: value === "#ffffffff" ? "none" : "tozeroy",
  ...
  const fillColorSchemeOnChange = (ev, item) => {
    const newProps = {
      areaFills: {
        ...props.areaProp,
        fillColorScheme: item.key,
        fill: item.key === "none" ? "none" : "tozeroy",
      }
    }
    changeProperties(newProps);
}
    ...
    <div className="insets-container area-fills">
      ...
      {/* <div className={'d-flex align-items-center'} style={{ marginBottom: '2px', marginLeft: '5px' }}>
        <Checkbox
          ...
          className="ms-lg1"
        />
        <Label className="ms-lg5">Identify intersections</Label>
      </div> */}
      {/* <div className="insets-header">
      ...
      <div className={'d-flex'} style={{ marginBottom: '2px', marginLeft: '5px' }}>
        <Label className="ms-lg2">ColorScheme</Label>
        <Dropdown
          selectedKey={endColorScheme}
          options={options.current.colorSchemeOptions}
          styles={dropdownStyles}
          onChange={fillColorSchemeOnChange}
          className="ms-lg2"
      ...
      {/* <div className={'d-flex align-items-center'} style={{ marginBottom: '2px', marginLeft: '5px' }}>
        ...
        <Dropdown
          placeholder="(None)"
          // selectedKey={style}
          options={options.current.directionOptions}
          styles={dropdownStyles}
          className={'ms-lg2'}
          disabled={true}
      ...
      <div className={'d-flex align-items-center'} style={{ marginBottom: '2px', marginLeft: '5px' }}>
        <Label className="ms-lg2">Direction</Label>
        <Dropdown
          defaultValue="Down"
